...
- **Access model (recommended):** keep the container **private** and serve files
  through **short-lived signed URLs (SAS)** generated by the app — avoid anonymous
  public-blob access.
- **If staff can UPLOAD files** (not just download): please enable **malware/AV
  scanning** (e.g. **Microsoft Defender for Storage**), and we will enforce a
  file-type allow-list and size limits in the app. Flag any **DLP / data-
  classification** policy these files must comply with.
- **If files are admin-curated only** (the awards team uploads, staff download):
  lower risk, but still private container + signed URLs.
- **Retention:** advise any retention/deletion policy the stored files must follow.

> Please confirm whether file uploads are **staff-facing** or **admin-only** — that
> determines whether AV scanning / DLP review is required.
